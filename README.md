...
`admin`은 웹 어드민, `run`은 one-shot 실행, `add connector`는 고급/호환 명령명으로 KakaoTalk 내부 연결을 직접 관리하고, `discord serve`/`telegram serve`/`kakao serve`는 플랫폼 워커를 직접 띄우는 명령이다. `--connector <name>`은 KakaoTalk 내부 연결 범위를 좁힐 때 쓰고, Discord/Telegram은 `--agent <agent-name>`의 플랫폼 토큰을 사용한다. Discord/Telegram 개인 대화는 채널 생성 시 `targetType=direct`로 저장되며, 내부 세션 경계는 일반 채널과 동일하게 `channel.name:role`이다.
