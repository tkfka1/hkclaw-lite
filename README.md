...
- KakaoTalk은 Admin에 내장된 **릴레이 서버 하나**와 여러 **KakaoTalk 채널**로 운영한다. `connector`/session은 설정·CLI 호환을 위한 내부 계층이다.
- 채널은 **대화가 들어갈 라우팅 단위**다. KakaoTalk 채널은 pairing code/연결 상태, 대상 방/사용자 필터, 워크스페이스, 실행 모드, role 매핑, 하네스 세션 경계를 가진다.
- 메시징 워커는 Discord/Telegram 에이전트 토큰 또는 KakaoTalk 채널의 내부 연결을 열고, 들어온 메시지는 채널이 정한 하네스로 라우팅한다. 채널은 수신 프로세스나 릴레이 서버가 아니라 라우팅 규칙이다.
...
- Discord/Telegram은 채널이 있으면 필요한 수신 워커가 자동으로 켜지고, KakaoTalk은 KakaoTalk 채널/플랫폼 에이전트가 있으면 하나의 릴레이 수신 워커가 켜진다.
- KakaoTalk은 Admin 내장 릴레이 서버 하나에 여러 **KakaoTalk 채널**을 페어링해 운영하고, Discord/Telegram 토큰은 각 에이전트 설정에서 관리한다.
...
- 예약 실행도 같은 runtime SQLite DB의 lease로 중복 실행을 막는다. Helm 기본처럼 단일 Pod/Recreate로 운영하면 별도 Kubernetes CronJob 없이 Admin 프로세스가 due schedule을 처리한다.
...
- GitOps로 KakaoTalk 수신을 항상 켜두려면 별도 Deployment보다 `workers.kakao.enabled=true` sidecar를 권장한다. 같은 Pod라서 state/workspace PVC를 안전하게 공유하고, `kakao serve` 하나가 KakaoTalk 채널의 내부 연결/legacy 에이전트 Kakao 설정과 채널 라우팅을 모두 처리한다.
...
`admin`은 웹 어드민, `run`은 one-shot 실행, `schedule`은 저장된 채널 프롬프트를 주기적으로 실행하는 예약 명령, `add connector`는 고급/호환 명령명으로 KakaoTalk 내부 연결을 직접 관리하고, `discord serve`/`telegram serve`/`kakao serve`는 플랫폼 워커를 직접 띄우는 명령이다. `--connector <name>`은 KakaoTalk 내부 연결 범위를 좁힐 때 쓰고, Discord/Telegram은 `--agent <agent-name>`의 플랫폼 토큰을 사용한다. Discord/Telegram 개인 대화는 채널 생성 시 `targetType=direct`로 저장되며, 내부 세션 경계는 일반 채널과 동일하게 `channel.name:role`이다.

### 예약 실행

예약은 Kubernetes CronJob처럼 외부에서 `run`을 반복 호출하지 않고, Admin 프로세스가 runtime SQLite DB를 폴링해 due schedule을 실행한다. 그래서 채널 history, role messages, token usage, outbox 기록이 일반 채널 실행과 같은 경로에 남는다.
...
- 예약은 `runtime_schedules`에 저장되고, 개별 실행은 `runtime_schedule_runs`에 남는다.
- 실행 중에는 SQLite lease와 heartbeat가 잡혀 같은 예약이 동시에 두 번 돌지 않는다.
- Admin 화면의 **예약** 메뉴에서 추가/수정/삭제/즉시 실행할 수 있다.
- `--skill <name>`을 넣으면 해당 이름을 스킬 힌트로 프롬프트에 포함한다. 실제 skill 본문은 에이전트 설정의 `skills` 경로를 통해 기존 prompt envelope에 주입된다.
- `HKCLAW_LITE_SCHEDULER=0`으로 Admin 내장 스케줄러 폴링을 끌 수 있다. 이 경우 외부 워커에서 `hkclaw-lite schedule tick`을 호출할 수 있다.
...
KakaoTalk 연동의 운영 모델은 **Admin 내장 릴레이 서버 1개 + KakaoTalk 채널 + 에이전트** 3개다. `connector`/session은 설정 파일과 기존 CLI 호환을 위한 내부 계층이며, 일반 운영자는 채널 카드에서 pairing code와 연결 상태만 보면 된다. 별도 릴레이 서버는 기본적으로 필요 없다.

### 필요한 것

- 외부에서 접근 가능한 hkclaw-lite Admin URL
  - 예: `https://your-domain.example`
  - 현재 배포 예: `https://hkclaw-lite.idc.hkyo.kr`
- Kakao i OpenBuilder Skill URL
  - `https://your-domain.example/kakao-talkchannel/webhook`
  - 현재 배포 예: `https://hkclaw-lite.idc.hkyo.kr/kakao-talkchannel/webhook`
- 응답을 만들 AI Agent
  - Codex/Claude/Gemini/local LLM/command 중 하나
- KakaoTalk Channel
  - pairing code/연결 상태와 어떤 Kakao 메시지를 어떤 workspace/agent/mode로 처리할지 정하는 라우팅 규칙
- Kakao relay receiving worker
  - 채널 화면의 KakaoTalk 릴레이 서버 영역에서 시작/재시작한다. 직접 실행할 때는 `hkclaw-lite kakao serve`를 쓴다.
...
2. Kakao i OpenBuilder의 Skill URL에 아래 주소를 등록한다.
   ...
   현재 Kubernetes 배포는 아래 주소를 쓰면 된다.

   ```txt
   https://hkclaw-lite.idc.hkyo.kr/kakao-talkchannel/webhook
   ```

3. 웹 어드민에서 AI 로그인 후 Agent를 만든다.
   - Agent는 실제 응답을 실행하는 주체다.
   - KakaoTalk 수신 자체는 채널이 소유하므로 Agent 플랫폼을 꼭 Kakao로 만들 필요는 없다.
...
- `add connector`와 `--connector <name>`은 기존 자동화/분리 운영을 위한 호환 표면이다. 일반 웹 어드민 운영에서는 채널 저장 시 내부 연결이 자동으로 생긴다.
...
- 채널마다 worker/Pod를 따로 만들 필요는 없다. 기본은 Admin 내장 릴레이 서버와 `kakao serve` 하나다.
- 외부 릴레이를 강제로 쓰고 싶을 때만 내부 연결의 릴레이 서버 URL을 별도 relay 주소로 지정한다.
...
- **KakaoTalk 내부 연결 / Connector**: KakaoTalk 전용 pairing/relay 상태를 보존하는 내부 호환 계층이다. 설정 파일과 CLI 명령에서는 `connector`라고 부른다. 일반 운영자는 직접 만들지 않고 KakaoTalk Channel이 자동으로 소유한다.
- **Channel**: Discord/Telegram/KakaoTalk 대상, KakaoTalk pairing 상태 또는 에이전트 플랫폼 설정, 기본 워크스페이스, 실행 모드, role 매핑을 가진 라우팅 단위다. 릴레이 서버를 새로 만드는 곳이 아니다.
...
- **Messaging worker**: Discord/Telegram/KakaoTalk 플랫폼 수신 프로세스다. Discord/Telegram은 Agent 카드에서, KakaoTalk 채널 기반 수신은 채널 화면의 KakaoTalk 릴레이 서버 영역에서 하나의 릴레이 수신 워커로 관리한다.
