...
- KakaoTalk은 Admin에 내장된 **릴레이 서버 하나**와 여러 **Kakao 세션**으로 운영한다. `connector`는 설정/CLI 호환 명칭이고 UI에서는 세션으로 표시한다.
- 채널은 **대화가 들어갈 라우팅 단위**다. Kakao 세션 또는 에이전트 플랫폼 설정, 대상 방/사용자 필터, 워크스페이스, 실행 모드, role 매핑, 하네스 세션 경계를 가진다.
- 메시징 워커는 Discord/Telegram 에이전트 토큰 또는 Kakao 세션을 열고, 들어온 메시지는 채널이 정한 하네스로 라우팅한다. 채널은 수신 프로세스나 릴레이 서버가 아니라 라우팅 규칙이다.
...
- Discord/Telegram은 채널이 있으면 필요한 수신 워커가 자동으로 켜지고, KakaoTalk은 Kakao 세션/플랫폼 에이전트가 있으면 하나의 릴레이 수신 워커가 켜진다.
- KakaoTalk은 Admin 내장 릴레이 서버 하나에 여러 **KakaoTalk 세션**을 페어링해 재사용하고, Discord/Telegram 토큰은 각 에이전트 설정에서 관리한다.
...
- GitOps로 KakaoTalk 수신을 항상 켜두려면 별도 Deployment보다 `workers.kakao.enabled=true` sidecar를 권장한다. 같은 Pod라서 state/workspace PVC를 안전하게 공유하고, `kakao serve` 하나가 설정된 Kakao 세션/legacy 에이전트 Kakao 설정과 채널 라우팅을 모두 처리한다.
...
2. 웹에서 AI 로그인부터 끝낸다.
3. 에이전트를 만든다.
4. KakaoTalk을 쓸 때만 세션을 준비한다. Kakao 플랫폼 에이전트를 만들면 같은 이름의 세션이 자동 생성되고, Discord/Telegram은 에이전트에 토큰을 둔다.
5. 대화 대상을 만든다. Discord/Telegram은 서버/그룹 채널 또는 개인 대화/DM 중 하나를 고르고, Kakao는 사용할 세션을 고른다.
...
KakaoTalk 연동은 **Admin 내장 릴레이 서버 1개 + Kakao 세션 + 라우팅 채널 + 에이전트** 조합으로 동작한다. `connector`는 내부/CLI 호환 명칭이고, 운영자가 이해해야 할 단위는 릴레이 서버가 아니라 세션이다. 별도 릴레이 서버는 기본적으로 필요 없다.
...
- KakaoTalk Session
  - Admin 내장 릴레이 서버에 페어링되는 Kakao 계정/세션 정보
- KakaoTalk Channel
  - 어떤 Kakao 세션의 어떤 메시지를 어떤 workspace/agent/mode로 처리할지 정하는 라우팅 규칙
...
3. 웹 어드민에서 AI 로그인 후 Agent를 만든다.
   - Agent 플랫폼을 Kakao로 만들면 같은 이름의 KakaoTalk 세션이 자동 생성된다.
   - 이 경우 채널에서 별도 KakaoTalk 세션을 다시 만들 필요 없이 자동 생성된 세션을 선택하면 된다.
4. 별도 계정/세션을 분리하고 싶을 때만 **채널** 화면의 KakaoTalk 릴레이 서버 영역에서 KakaoTalk 세션을 추가로 만든다.
   - 이름: 예 `kakao-main`
   - 릴레이 서버 URL: 예 `https://your-domain.example/`
   - 페어링 토큰/세션 토큰: 선택값. 비우면 pairing code를 발급한다.
5. KakaoTalk Channel을 만든다.
   - KakaoTalk 세션: 위에서 만든 `kakao-main` 또는 Kakao 플랫폼 Agent에서 자동 생성된 세션
   - `Kakao 수신 channelId 필터`: 보통 `*`
   - `Kakao 사용자 ID 필터`: 특정 사용자만 받을 때만 입력
   - owner/reviewer/arbiter agent, workspace, single/tribunal mode 설정
6. KakaoTalk 릴레이 서버 영역에서 pairing code를 확인한다. 자동 시작이 꺼져 있으면 릴레이 수신 워커를 시작/재시작한다.
...
- `hkclaw-lite kakao serve` 하나가 설정된 모든 Kakao 세션/채널 라우팅을 처리할 수 있다.
- 특정 Kakao 세션만 분리 운영할 때만 호환 옵션명인 `--connector <name>`을 쓴다.
- Helm/GitOps에서 상시 수신이 필요하면 `workers.kakao.enabled=true`로 같은 Pod 안의 sidecar를 켜는 구성을 권장한다.

### 릴레이 서버 URL과 엔드포인트

Kakao 세션의 릴레이 서버 URL은 Kakao 릴레이 수신 워커가 붙을 Admin 내장 relay 주소다. 보통 Admin 외부 URL을 그대로 넣는다.
...
- Kakao 세션은 KakaoTalk 접속/세션 정보이고, Channel은 라우팅/워크스페이스/역할 설정이다.
- Kakao 플랫폼 Agent에서 자동 생성된 세션은 Agent Kakao 설정이 원본이다. 채널에서는 이 세션을 재사용만 하면 된다.
- `kakaoChannelId`의 `*`는 모든 channelId를 받는다는 뜻이다.
- 같은 Kakao 세션 안에서 라우팅 필터가 겹치는 Channel은 저장되지 않는다.
- 세션 하나로 여러 Channel을 처리할 수 있지만, `kakaoChannelId`나 `kakaoUserId`로 메시지가 정확히 하나의 Channel에만 매칭되게 나눠야 한다.
- 채널마다 worker/Pod를 따로 만들 필요는 없다. 기본은 Admin 내장 릴레이 서버와 `kakao serve` 하나다.
- 외부 릴레이를 강제로 쓰고 싶을 때만 Kakao 세션의 릴레이 서버 URL을 별도 relay 주소로 지정한다.
...
- **KakaoTalk Session / Connector**: KakaoTalk 전용 계정/세션이다. 설정 파일과 CLI 명령에서는 호환성을 위해 `connector`라고 부르지만, UI에서는 세션으로 표시한다. 타입은 `kakao`로 고정이고, 여러 Channel이 같은 Kakao 세션을 공유할 수 있다.
- **Channel**: Discord/Telegram/KakaoTalk 대상, Kakao 세션 또는 에이전트 플랫폼 설정, 기본 워크스페이스, 실행 모드, role 매핑을 가진 라우팅 단위다. 릴레이 서버나 세션을 새로 만드는 곳이 아니다.
...
- **Messaging worker**: Discord/Telegram/KakaoTalk 플랫폼 수신 프로세스다. Discord/Telegram은 Agent 카드에서, Kakao 세션 기반 수신은 채널 화면의 KakaoTalk 릴레이 서버 영역에서 하나의 릴레이 수신 워커로 관리한다.
